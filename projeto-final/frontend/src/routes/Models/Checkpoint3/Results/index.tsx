...
export function setTaskData(data: any){
    localStorage.setItem("taskData_checkpoint3", JSON.stringify(data));
}
...
export default function Checkpoint3ResultsPage(){
    ...
    useEffect(() => {
      if (localStorage.getItem("taskData_checkpoint3") !== null) {
        setInvalidData(false);
        setLocalData(JSON.parse(String(localStorage.getItem("taskData_checkpoint3"))));
      } else {
        console.log("invalid data")
        setInvalidData(true);
        navigate('/models/checkpoint3/')
      }
    }, [change]);
    ...
    const downloadFile = ({ data, fileName, fileType }: any) => {
      ...
      const exportToJson = (e: any, result: any) => {
        ...
        downloadFile({
          ...
          fileName: 'simulation_checkpoint3.json',
          fileType: 'text/json',
        })
      }

      function handleClickNewSim() {
        localStorage.setItem("taskData_checkpoint3", "");
        navigate("/models/checkpoint3");
      }
