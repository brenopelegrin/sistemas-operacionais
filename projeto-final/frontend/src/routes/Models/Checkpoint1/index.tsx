...
import { make_checkpoint1_post, makeGet } from '../../../components/TaskFetch';
...
export default function Checkpoint1Page() {
    ...
    onSuccess: async (data, variables) => {
        ...
        if(status != 'SUCCESS'){
            ...
            if (!isEmpty(data["data"]["result"])){
                ...
                navigate('/models/checkpoint1/results');
            } else {
                await sleep(500);
                mutateGet({id}); 
            }
  ...
  const {mutate: mutatePost, data: dataPost, error: errorPost, isError: isErrorPost, isSuccess: isSuccessPost} = useMutation(make_checkpoint1_post,
    {
  ...
  return (
    <Center>
      {controlsVisible ?     <CardWithButton header="Controles - checkpoint1" minWidth="1vw" customButton={myButton}>
